...
public class StreamAlerter {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(trieTree.search("赌"));
    }

    public boolean query(char ch) {
        return trieTree.search(String.valueOf(ch));
    }
}
...
class TrieTree{
    ...
    public TrieTree(char c){
        ringBuffer=new RingBuffer<String>(100);
        stringBuffer=new StringBuffer();
        root=new TrieNode(c,new HashMap<Character, TrieNode>());
    ...
    public boolean search(String str){
        ...
        for(int i=0;i<stringBuffer.toString().length();i++){
            TrieNode child=cur.nodeMap.get(stringBuffer.toString().charAt(i));
            if(child==null){
